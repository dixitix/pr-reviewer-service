...
Запуск локально: `make load-test` — поднимет compose, очистит БД, зальёт данные для нагрузочного теста и запустит Locust (`test/load/locustfile.py`). Требуются `python3` и установленный `locust`.
